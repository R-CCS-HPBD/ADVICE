...
//                if(!v.second.cp) continue;
    ...
    if (file2.is_open()) {
        for (const auto& var : cher_CPVar) {
            auto v = var.second;
            if(v.vvname!="") {
                file2<<v.ffname<<","<<v.vvname<<","<<v.state<<endl;
            }
            else {
                file2<<v.fname<<","<<v.vname<<","<<v.state<<endl;
            }
    ...
    std::ofstream file4(LOADFILEADDR+cher_dataFiles[7]);//"cpVar.txt"
    if (file4.is_open()) {
        for (const auto& var : cher_CPVar) {
            auto v = var.second;
            ...
            if(v.type == "pointer" || v.type == "array"){
                file4 << v.fname << "," << v.vname << ","
                      << cher_arrVarsMap[v.addr].vartype << ","
                      <<cher_arrVarsMap[v.addr].elemsTotal << ","<<endl;
            }else{
                file4 << v.fname << "," << v.vname << ","
                      << cher_checkpointList[v.addr].type << ","
                      <<1 << ","<<endl;
            }
...
void cher_arrayidx_double(double idx){
    //std::cout<<idx<<" : "<<*idx<<endl;
    //std::cout<<"dl : "<< idx<<endl;
}

void cher_arrayidx_float(float idx){
    //std::cout<<idx<<" : "<<*idx<<endl;
    //std::cout<<"fl : "<<idx<<endl;
}
...
void cher_arrLenUsed(int flag,int bbNum,int start){
    ...
    ///再判断个数
    if(varr.elemsTotal > 0 && (double)varr.elemsWrite/(double)varr.elemsTotal>=0.6 && varr.elemsRead == 0){
        ///全覆盖
        cher_arrVarsMap[addr0].user = "in";
    }else if(varr.elemsWrite == varr.elemsRead  && varr.elemsRead>0){/// a[1] = 1;b[1] = a[1];
        ///如果 数组元素被写 与 读的次数相等 ,则为全覆盖(数组元素被读过)
        cher_arrVarsMap[addr0].user = "in";
    }
    /// int a[10] , a[1-5]= 0 ; a[5-10];
