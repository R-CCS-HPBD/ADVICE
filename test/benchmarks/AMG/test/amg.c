      ...
      char *argv[] )
{
   ...
   HYPRE_Solver        pcg_solver; ///struct
...
//          printf("1\n");
         ...
         HYPRE_BoomerAMGCreate(&pcg_precond); /// init
         HYPRE_BoomerAMGSetTol(pcg_precond, pc_tol);
         HYPRE_BoomerAMGSetCoarsenType(pcg_precond, coarsen_type);
         HYPRE_BoomerAMGSetPMaxElmts(pcg_precond, P_max_elmts);
         HYPRE_BoomerAMGSetPrintLevel(pcg_precond, poutdat);
...
//          printf("4\n");
         for (i=0; i < 4; i++)
         {
            ...
            HYPRE_GMRESSolve(pcg_solver, (HYPRE_Matrix)parcsr_A, (HYPRE_Vector)b, (HYPRE_Vector)x); /// main loop
            HYPRE_GMRESGetNumIterations(pcg_solver, &num_iterations);
            HYPRE_GMRESGetFinalRelativeResidualNorm(pcg_solver,&final_res_norm);
            if (myid == 0 && print_stats)
            {
         ...
         HYPRE_BoomerAMGDestroy(pcg_precond);/// destroy
...
//    printf("local_size  %d = last_row %d - first_row %d +1 \n",local_size,last_row,first_row);
   num_cols = hypre_CTAlloc(HYPRE_Int, local_size); /// 1
//    printf("a2\n");
   row_nums = hypre_CTAlloc(HYPRE_Int, local_size); /// 2
//    printf("a3\n");
   col_nums = hypre_CTAlloc(HYPRE_Int, local_size);/// 3
//    printf("a4\n");
   data = hypre_CTAlloc(HYPRE_Real, local_size);/// 4
//    printf("a5\n");
#ifdef HYPRE_USING_OPENMP
